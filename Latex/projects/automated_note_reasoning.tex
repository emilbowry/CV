...
% \textbf{Automated Notes Reasoning:} 
% A knowledge graph inference engine reverse-engineered into the Obsidian Markdown Editor.
% \begin{itemize}
% 	\item Enforced Universal Properties (Categorical Products)  to programmatically identify missing intersectional nodes between disjoint topics, auto-generating necessary bridging notes.
% 	\item Modelled the relationship between Syntax and Semantics as an **Adjunction** (Galois Connection). This mathematically guaranteed that every grammatical title mapped to a logical object, and every logical gap triggered a specific syntax generation.
% 	\item Applied the Yoneda Lemma to classify notes solely via their morphisms to specific `Type Functors' (Definitions, Equations), allowing the system to infer note properties without parsing internal content.
% \end{itemize}
